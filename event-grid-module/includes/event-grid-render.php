...
if (!class_exists('Event_Grid_Render')) {
    class Event_Grid_Render {
        public static function render_grid($props) {
            ...
            if (!empty($props['filter_category'])) {
                $tax_query[] = array(
                    'taxonomy' => 'evenements_category',
                    'field'    => 'term_id',
                    'terms'    => is_array($props['filter_category']) ? $props['filter_category'] : array($props['filter_category']),
                );
            }

            if (!empty($props['filter_geozone'])) {
                $tax_query[] = array(
                    'taxonomy' => 'relation_geozone',
                    'field'    => 'term_id',
                    'terms'    => is_array($props['filter_geozone']) ? $props['filter_geozone'] : array($props['filter_geozone']),
                );
            }
            ...
            if (empty($posts)) {
                $output .= '<div class="debug">❌ Aucun post trouvé</div>';
            } else {
                foreach ($posts as $post) {
                    ...
                    // Fallback to category image if post thumbnail is missing
